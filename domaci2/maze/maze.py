...
if __name__ == "__main__":
    ...
    env = MazeEnvironment(board, gamma=0.9)
    agent = Agent(env, Action.get_all_actions(), state=(0, 0))
    agent.set_policy(GreedyPolicy())

    # print(Policy.defined_policies)
    Draw.print_probabilities(agent)
    ...
    Draw.draw_policy(agent, 'greedy_v', ax=axes[2])
    axes[2].set_title('Optimal policy using V values')

    Draw.draw_policy(agent, 'greedy_q', ax=axes[3])
