...
class Plot(ABC):
    ...
    def plot(
        self,
        env: BanditsEnvironment,
        CHANGE_AT: List[int] = None,
        old_bandit_mean: List[List[float]] = None,
    ) -> None:
        pass
...
class RewardPlot(Plot):
    ...
    def plot(
        ...
        plt.scatter(
            range(len(env.bandits)),
            [env.bandits[i].mean for i in range(len(env.bandits))],
            marker="x",
        )
...
class ConvergencePlot(Plot):
    ...
    def __init__(
        self, q_evol: List[Dict[int, List[float]]], eps: float, ATTEMPTS_NO: int
    ) -> None:
        self.q_evol = q_evol
        self.eps = eps
        ...
        for i, bandit in enumerate(env.bandits):
            if CHANGE_AT:
                for j in range(len(CHANGE_AT)):
                    ...
                    start = CHANGE_AT[j]
            plt.plot(
                range(CHANGE_AT[-1] if CHANGE_AT else 0, self.ATTEMPTS_NO),
                bandit.mean
                * np.ones(
                    self.ATTEMPTS_NO - CHANGE_AT[-1] if CHANGE_AT else self.ATTEMPTS_NO
                ),
                linestyle="--",
                label="_nolegend_",
                color=colors[i],
            ...
            plt.scatter(
                [i for i in range(self.ATTEMPTS_NO + 1)],
                self.q_evol[i],
                marker=".",
                s=10,
                color=colors[i],
...
class System:
    ...
    def __init__(
        self, bandits: List[Bandit], stationary: bool = True, ALPHA: float = 0.1
    ):
        self.env = BanditsEnvironment(bandits, stationary=stationary)
        self.ALPHA = ALPHA

    def run_system(
        self, eps: float = 0, ATTEMPTS_NO: int = 10000, CHANGE_AT: List[int] = None
    ):
        if CHANGE_AT:
        ...
        if not self.env.stationary:
            ...
            plotter.plot(
                env=self.env, CHANGE_AT=CHANGE_AT, old_bandit_mean=old_bandit_mean
            )
        else:
            plotter = RewardPlot(rewards=rewards, q=q)
